...
def main() -> int:
    ...
    if not DATA_FULL_JSON.exists():
        with DATA_FULL_JSON.open("w", encoding="utf-8") as f:
    ...
    for species, entry in full_pool.items():
        if species in EXCLUDED_UBERS:
            continue
        if species in ALWAYS_KEEP:
            kept[species] = entry
            continue
        tier = GEN1_TIER_RATINGS.get(species)
        if tier is None:
            print(f"  WARN: no tier rating for {species!r} — excluding")
            continue
        if tier >= MIN_TIER:
            kept[species] = entry
